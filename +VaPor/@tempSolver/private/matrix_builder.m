    ...
    for J=1:size(obj.flow_obj.tissue,2)
        for K=1:size(obj.flow_obj.tissue,3)
            if obj.flow_obj.tissue(I,J,K)
                ...
                if ~isinf(obj.Beta.porous) % If the heat transfer coefficient is finite.
                    if Porosity_b ~= 0 % If there are two phases present.
                       T_DataTempPos = T_DataTempPos + 1; T_DataTemp(T_DataTempPos,:) = [Row,Row,-obj.Beta.porous]; % Tissue domain interaction for current voxel (tissue domain).
                        ...
                        T_DataTempPos = T_DataTempPos + 1; T_DataTemp(T_DataTempPos,:) = [BRow,Row,1]; % Blood domain interaction for current voxel (tissue domain).
                        T_DataTempPos = T_DataTempPos + 1; T_DataTemp(T_DataTempPos,:) = [BRow,BRow,-1]; % Blood domain interaction for current voxel (blood domain).
                ...
                %                     T_DataTemp = [T_DataTemp;BRow,BRow,-Vol*Porosity_b*obj.flow_obj.bloodDensity*obj.bloodCp/Timestep];
                %                 end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if ~isempty(obj.flow_obj.Volume2VesselArt{I,J,K}) % If this voxel has intersecting artery segments.
                    for mn = 1:size(obj.flow_obj.Volume2VesselArt{I,J,K},1) % For every intersecting artery segment.
                        ...
                        if obj.flow_obj.SplitArt(Node,1) == 1
                            ...
                            T_DataTempPos = T_DataTempPos + 1;
                            T_DataTemp(T_DataTempPos,:) = [BRow,NumDomRows+Node,obj.flow_obj.SplitArt(Node,2)*NodeMdot*obj.bloodCp]; % Artery domain interaction for node.
                            T_DataTempPos = T_DataTempPos + 1;
                            T_DataTemp(T_DataTempPos,:) = [BRow,NumDomRows+NodeConn,(1-obj.flow_obj.SplitArt(Node,2))*NodeMdot*obj.bloodCp]; % Artery domain interaction for connecting node.
                            T_DataTempPos = T_DataTempPos + 1;
                            T_DataTemp(T_DataTempPos,:) = [BRow,BRow,-NodeMdot*obj.bloodCp]; % Blood domain interaction for current voxel.
                            ...
                            T_DataTempPos = T_DataTempPos + 1;
                            T_DataTemp(T_DataTempPos,:) = [BRow,NumDomRows+NodeUp,NodeMdot*obj.bloodCp]; % Blood domain interaction for upstream arterial node.
                            T_DataTempPos = T_DataTempPos + 1;
                            T_DataTemp(T_DataTempPos,:) = [BRow,BRow,-NodeMdot*obj.bloodCp]; % Blood domain interaction for current voxel.
...
for Node = 1:size(obj.flow_obj.Vessel2VolumeArt,1)
    ...
    if ismember(Node,obj.flow_obj.arteries.termPoints)
        ...
        %%%%%% Transient Heat Transfer %%%%%%%%%%%%%%%%%%
        %     if Option_TransientSolve
        %         T_DataTemp = [T_DataTemp;NumDomRows+Node,NumDomRows+Node,-Vol1(Node)*obj.flow_obj.bloodDensity*obj.bloodCp/Timestep];
        %     end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for Node = 1:size(obj.flow_obj.Vessel2VolumeVein,1)
    ...
    %%%%%% Transient Heat Transfer %%%%%%%%%%%%%%%%%%
    %     if Option_TransientSolve
    %         T_DataTemp = [T_DataTemp;NumDomRows+VesselRow+Node,NumDomRows+VesselRow+Node,-Vol2(Node)*obj.flow_obj.bloodDensity*obj.bloodCp/Timestep];
    %     end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
