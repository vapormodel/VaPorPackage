function newStrokeAdjustment(obj)
...
img = 1 - obj.strokeMask;
img(~obj.grey_white) = 0;
distance = bwdist(img);
distance(~obj.grey_white)= nan;
distance = 1 - distance/max(distance, [],'all');

% Threshold the distances
distance(distance<0.5)=0;
distance(~obj.strokeMask) = nan;

% Rescale the distances: 
upp_lim = 1;
low_lim = 0.2;
distance(distance ~= 0) = (upp_lim-low_lim)*(distance(distance ~= 0)-min(distance(distance ~= 0)))./(max(distance(distance ~= 0))-(min(distance(distance ~= 0)))) + low_lim;

total_weighting = sum(distance(obj.strokeMask));

vein_flow_normal = sum(obj.MdotVoxelsVein(~obj.strokeMask));

venous_flow_to_assign = -(sum(obj.MdotVoxelsArt - obj.deductions, 'all') + vein_flow_normal);

adjustedflow = distance * (venous_flow_to_assign/total_weighting);

obj.MdotVoxelsVein(obj.strokeMask) = adjustedflow(obj.strokeMask);

obj.MdotVoxelsArt = obj.MdotVoxelsArt - obj.deductions;
